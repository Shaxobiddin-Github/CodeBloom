...
class Video(models.Model):
    ...
    )  # Agar playlist kerak bo'lmasa, null=True va blank=True qo'shiladi
    author = models.ForeignKey(
        Teacher, on_delete=models.CASCADE, related_name='videos'
    )  # Videoni yaratgan foydalanuvchi
    title = models.CharField(max_length=200)  # Videoning sarlavhasi
    likes = models.PositiveIntegerField(default=0)  # Layklar soni
    created_at = models.DateTimeField(auto_now_add=True)  # Yaratilgan sana
    video_file = models.FileField(upload_to='videos/', null=True, blank=True)  # Videoning fayli
